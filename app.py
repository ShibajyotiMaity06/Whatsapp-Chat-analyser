...
uploaded_file = st.sidebar.file_uploader("Choose Only a whatsapp Chat exported file")
...
if uploaded_file is not None:
    if uploaded_file.name.endswith(".txt"):
        ...
        st.title('Import Correct Whatsapp File')
        st.text('''Procedure to analyse the chats:
                1)First export chat without media from whatsapp 
                2) Download that folder extract it 
                3) In browse files select the chat from extracted folder 
                4) Then you go Boom!''''')

else:
    st.title('Firstly select a file')
    st.text('''Procedure to analyse the chats:
            1)First export chat without media from whatsapp 
            2) Download that folder extract it 
            3) In browse files select the chat from extracted folder 
            4) Then you go Analyse you chats!''''')
